...
import { StyleSheet, Text, View, TouchableOpacity, Image, SafeAreaView, Modal, ScrollView, TextInput } from "react-native"
    ...
    const getOwnerName = async () => {
        try {
            if (selectedListing) {
                ...
                const data = onSnapshot(nameQuery, (snapshot) => {
                    if (snapshot.docs.length > 0) {
                        setOwnerName(snapshot.docs[0].data().name);
                    } else {
                        setOwnerName("");
                    }
                })
